...
function _extract_bilinear_onsite(model::SLCEModel)
...
"""
    ExchangeModel(model::SLCEModel) -> ExchangeModel
...
"""
    MultipoleModel(model::SLCEModel) -> MultipoleModel

Digest a fitted `SLCEModel` into the full-multipole mean field (P4): one mean-field term per
...
averages `⟨Z_lm⟩`.
"""
function MultipoleModel(model::SLCEModel)
...
"""
MFASampler(model::SLCEModel; reference::AbstractMatrix{<:Real}) =
